  ...
  batchUpdate() {
    ...
    if (PERHIASAN.length != 0) {
      for (let p of PERHIASAN) {
        no++;
        perhiasan[no] = btoa(
          JSON.stringify({ _id: p.detail._id, flag: "sale" })
        );
    ...
    if (LM.length != 0) {
      for (let Lm of LM) {
        no++;
        lm[no] = btoa(JSON.stringify({ _id: Lm.detail._id, flag: "sale" }));
    ...
    if (GS.length != 0) {
      for (let Gs of GS) {
        no++;
        gs[no] = btoa(JSON.stringify({ _id: Gs.detail._id, flag: "sale" }));
    ...
    if (BERLIAN.length != 0) {
      for (let b of BERLIAN) {
        no++;
        berlian[no] = btoa(JSON.stringify({ _id: b.detail._id, flag: "sale" }));
    ...
    if (DINAR.length != 0) {
      for (let d of DINAR) {
        no++;
        dinar[no] = btoa(JSON.stringify({ _id: d.detail._id, flag: "sale" }));
