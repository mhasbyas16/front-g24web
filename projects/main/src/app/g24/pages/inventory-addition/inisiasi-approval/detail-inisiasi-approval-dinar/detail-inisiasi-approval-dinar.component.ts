...
import { DetailItemInisiasiApprovalDinarComponent } from './detail-item-inisiasi-approval-dinar/detail-item-inisiasi-approval-dinar.component';
...
@Component({
  selector: 'detail-inisiasi-approval-dinar',
  templateUrl: './detail-inisiasi-approval-dinar.component.html',
  styleUrls: ['./detail-inisiasi-approval-dinar.component.scss']
})
export class DetailInisiasiApprovalDinarComponent extends BasePersistentFields implements OnInit {

  @ViewChild('product') product : ElementRef;
  
  @ViewChild('item_terima') modalItemTerima : DetailItemInisiasiApprovalDinarComponent;
  btoa = btoa;
  parseInt = parseInt;
  console = console;
  Object = Object;
  ...
  async LoadProductCategory()
  {
    ...
    let products = await this.productCatService.list("?code=c06").toPromise();
  ...
  async LoadVendor()
  {
    let vendors = await this.vendorService.list("?product-category.code=c06").toPromise();
  ...
  onProductChanged()
  {
    ...
    {
      let dinar = this.products[i];
      if(dinar.code == "c06")
      {
        this.input['product-category'] = dinar;
        break;
      }
    }
  }
  ...
  doSearch(form : FormGroup)
  {
    ...
    let params = "?product-category.code=c06" + create_p + order_status_p + unit_p + no_po_p;
