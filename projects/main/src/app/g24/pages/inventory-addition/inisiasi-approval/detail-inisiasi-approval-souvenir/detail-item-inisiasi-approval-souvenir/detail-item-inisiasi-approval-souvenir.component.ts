...
@Component({
  selector: 'detail-item-inisiasi-approval-souvenir',
  templateUrl: './detail-item-inisiasi-approval-souvenir.component.html',
  styleUrls: ['./detail-item-inisiasi-approval-souvenir.component.scss']
})
export class DetailItemInisiasiApprovalSouvenirComponent implements OnInit {
  ...
  constructor(
    ...
    private title : string = "Detail Approval Souvenir";
    public get Title()
    {
      return this.title;
    }
  ...
  {
    this.jurnalInisiasi.bayarSouvenir(idInisiasi).subscribe(output => {
      if(output == false)
      {
    ...
    getOngkosFromItems()
    {
      ...
      for(let i = 0; i < this.inisiasi.items.length; i++)
      {
        if(this.inisiasi.items[i]?.ongkos_pieces == null || this.inisiasi.items[i]?.ongkos_pieces == "null")
          continue;
          value += parseFloat(this.inisiasi.items[i].ongkos_pieces);
      }
