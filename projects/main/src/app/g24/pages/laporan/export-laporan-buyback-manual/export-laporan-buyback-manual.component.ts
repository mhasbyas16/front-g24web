...
@Component({
  selector: 'app-export-laporan-buyback-manual',
  templateUrl: './export-laporan-buyback-manual.component.html',
  styleUrls: ['./export-laporan-buyback-manual.component.scss']
})
export class ExportLaporanBuybackManualComponent implements OnInit {
    ...
    if (data.product.PERHIASAN != null) {
      ...
      if (data.product.LM  !=  null) {
        ...
        for (let lm of data.product.LM) {
          this.noItem++;
          rowLM.push([this.noItem, lm.detail.vendor.name, lm.detail['product-denom'].name, 'RP. '+ new Intl.NumberFormat(['ban', 'id']).format(lm.hargaBB) ])
        }
