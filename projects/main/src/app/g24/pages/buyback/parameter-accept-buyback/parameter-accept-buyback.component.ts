...
@Component({
  selector: 'app-parameter-accept-buyback',
  templateUrl: './parameter-accept-buyback.component.html',
  styleUrls: ['./parameter-accept-buyback.component.scss'],
  providers:[DatePipe],
})
@DContent(ParameterAcceptBuybackComponent.key)
export class ParameterAcceptBuybackComponent implements OnInit {
  ...
  dataList(){
    ...
    this.buybackAcceptParameterService.list("?_hash=1&_sortby=flag:1&_between=makerDate&_start=" + form + "&_end=" + to).subscribe((response:any)=>{
  ...
  filterTransaction(){
    ...
    this.buybackAcceptParameterService.list("?_hash=1&_sortby=flag:1&_between=makerDate&_start=" + from + "&_end=" + to + flag).subscribe((response:any)=>{
      if (response == false || response["length"] == 0) {
        this.toastrService.error("Data Not Found");
        this.dataSearch = [];
        return;
  ...
  form(){
    this.formAdd = new FormGroup ({
      ...
      maxPrm: new FormControl ("", [Validators.required, Validators.pattern(/^[0-9]\d*$/)]),
    });
  }
  ...
  rejectParameter(){
    ...
    this.buybackAcceptParameterService.update(data).subscribe((response:any)=>{
      if (response == false) {
  ...
  approveParameter(){
    ...
    this.buybackAcceptParameterService.get(params).subscribe((response:any)=>{
      if (response == false) {
  ...
  storeParameter(){
    ...
    this.buybackAcceptParameterService.add(data).subscribe((response:any)=>{
      if (response == false) {
  ...
  static key = EMenuID.PARAMETER_ACCEPT_BUYBACK
}
